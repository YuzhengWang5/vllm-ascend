...
_ORIG_GET_KV_CACHE_COORDINATOR_PARAMS = set(
    inspect.signature(_orig_get_kv_cache_coordinator).parameters
)
...
def get_kv_cache_coordinator(
    ...
    if len(kv_cache_config.kv_cache_groups) == 1 or not enable_caching:
        ...
        if "max_in_flight_tokens" in _ORIG_GET_KV_CACHE_COORDINATOR_PARAMS:
            orig_kwargs["max_in_flight_tokens"] = token_budget
        if "scheduler_block_size" in _ORIG_GET_KV_CACHE_COORDINATOR_PARAMS:
