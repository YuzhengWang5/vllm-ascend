...
def serve(args: argparse.Namespace) -> None:
    ...
    # All captured layer/batch graphs execute serially on the same stream.
    # Reuse one allocator pool, as vLLM's own ACLGraph runner does, rather
    # than reserving a separate workspace for every one of the 61 x N graphs.
    # The latter exhausts a 64-GiB indexer die when the full 128K index cache
    # is present even though the live workspace of one graph is much smaller.
